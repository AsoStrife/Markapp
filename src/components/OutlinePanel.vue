        ...
        <li v-for="({ it, pos }, i) in visibleItems" :key="it.index">
          <div class="flex items-center">
            <button v-if="hasChildrenAt(pos)"
              ...
              @click.stop="toggleCollapse(it.index)"
              :title="collapsed[it.index] ? 'Expand' : 'Collapse'">
              <svg class="w-4 h-4 transition-transform" :style="{ transform: collapsed[it.index] ? 'rotate(-90deg)' : 'rotate(0deg)' }" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
              </svg>
            </button>
            <button class="flex-1 text-left px-3 py-1.5 hover:bg-gray-700 rounded transition-colors text-gray-200"
